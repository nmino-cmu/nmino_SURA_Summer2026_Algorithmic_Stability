...
%% Annotated twin: cyan decision notes (not live math).
\newcommand{\annote}[1]{\textcolor{cyan}{\textbf{[Annotation]:} #1}}
...
\title{Algorithmic Stability for Optimization-Selected Uncertainty Quantification (Annotated)}
...
\maketitle
Annotated twin of the formal problem statement.
The mathematical objects, equations, Part~I/II split, goals~(i)--(iii), and symbol table match the clean writeup at
\texttt{Formulation/writeup/Problem\_Writeup.tex}.
Cyan \texttt{[Annotation]:} blocks record why those choices were locked (Wenbin H2 tags W1--W5, July~2 email, and meeting notes).
Annotations are not axioms and do not enlarge the live math.
\begin{abstract}
...
\textbf{How to read this file.}\\[0.35em]
Black text $=$ live formal problem (same math as the clean twin).\\
Cyan \texttt{[Annotation]:} $=$ decision rationale (quotes from Wenbin / transcript / email).\\
Do not treat annotations as additional problem objects.
}}
\end{center}
...
\annote{Decision: lock the scientific intersection as post-hoc inference $\cap$ algorithm stability $\cap$ optimization theory, and treat that as the framing for Part~I (stability of constrained opt) + Part~II (post-hoc on top).
Wenbin (H2 / W1): ``The problem we study falls in the intersection of post-hoc inference, algorithm stability, and optimization theory.''}

\annote{Decision: narrow the live problem from general post-hoc selection to one objective --- uncertainty quantification --- via Wenbin's own UQ example (equations~\eqref{eq:validity}--\eqref{eq:opt} below); do not keep a co-equal general selector $\widehat S$ as live law.
Wenbin (H2 / W2): ``The post-hoc inference problem is itself very general. We need to narrow the structure of the problem by focusing on one specific objective (e.g., uncertainty quantification). See below for an example setting that I wrote.''
Consequently the general $\widehat S=A(D)$ framework from early brainstorming is not live: it is the broad class Wenbin told us to shrink, not the object we study.}

\annote{Attribution guard (Woody Meeting~3 vs Wenbin H2): the optimization-induced policy product agenda was primarily Woody's (M3).
Wenbin's binding narrowing of the live math is H2 (W1--W5) + the July~2 email (UQ example + methodology brainstorm) --- not Woody's M3 product vision.
Do not attribute Woody's product vision (general data-driven policy as the problem definition) as Wenbin's live problem.}

\annote{Earlier mentoring (Meeting~2), not the final narrow problem: Wenbin recommended building background in conformal prediction and optimization.
That advice orients reading and technique; it does not redefine the live objects beyond the H2 UQ example.}
...
\annote{Notation quirk: why the third slot of $\mathcal{C}$ is $\theta$ (Eq.~\eqref{eq:validity}) despite prose writing $\mathcal{C}(\ldots,\alpha)$.
Decision: follow the displayed validity equation, not the inconsistent prose arity --- third argument $=$ hyperparameter $\theta$; $\alpha$ stays the nominal miscoverage level in the coverage statement.
Silent ``fix $\alpha$ into the third slot'' would conflate the selection target with the coverage budget.}
...
\annote{Notation quirk: $y\neq Y$ unless a later lemma says so.
Decision: keep lowercase $y$ as a placeholder argument of $\hat f$, and keep uppercase $Y$ as the test response in~\eqref{eq:validity}; do not silently equate them.
Equating them without an explicit identification would smuggle a structural assumption Wenbin did not write.}
...
\annote{Decision: answer goal~(iii) by data-randomizing selection, not by recalibrating $\mathcal{C}$.
Recency: W5 (after the goal block on H2 p.3) overrides the body verb ``recalibrate.''
Strength: intended path \emph{now} (bias), not a forever metaphysical ban --- W4 still allows a later more principal setting.
Wenbin (H2 / W5): ``Data-driven policy (e.g., PTO), no recalibration, strategy: data-randomize to achieve validity guarantee, this answers (iii). Our paper: structured noise, depends on inverse optimization. Post-inference Tijana (Winner's Curse, Algorithm Stability).''
Primary live path $=$ data-randomize the map $\mathcal{D}\mapsto\theta$; keep the fixed-$\theta$ construction of $\mathcal{C}$ unchanged.}

\annote{Decision: treat the UQ example as provisional but currently binding --- replaceable only if a more principal version is later endorsed; W4 is not a present license to widen.
Wenbin (H2 / W4): ``We may modify the problem setting along the way if we find there are more principal versions of it.''}
...
\annote{Decision: split the research into Part~I (replace-one / one-observation movement of $\hat\theta$ from~\eqref{eq:opt}, plus structural assumptions on $\hat f,\hat g$) and Part~II (post-hoc inference on top).
Wenbin (H2 / W3): ``This objective is a mix of algorithm stability, post-hoc inference, and optimization theory. I believe that we should decompose the question into two main parts: first, study the stability of constrained optimizations (\ie, if the dataset changes by one data point, how much does the output of the optimization change? Do we need to make any structural assumptions on the optimization problems, like how $\hat f$ and $\hat g$ are learned from the data $\mathcal{D}$?). Second, study post-hoc inference on top of these results.''}
...
\annote{Decision: no Part~III. W3 asks for exactly two parts; goal~(iii) is a Part~II post-hoc question (restore~(1)), answered by methods nested under Part~II, not a co-equal third research half.}
...
\annote{Decision: methodologies may be drafted into the writeup, but brainstorming is authorized against the H2/July~2 problem setup --- not as a license to reinvent the problem.
July~2 email (Wenbin): ``I drafted a version of the problem setup in the Overleaf project, which aligns closer to what we intended.... begin brainstorming the methodologies\ldots''
(H2 UQ block stands in for the unrecovered Overleaf body; methodology brainstorm is downstream of that setup.)}
...
\annote{Decision: primary path for~(iii) is data-randomize selection at fixed $\mathcal{C}$, not recalibrate $\mathcal{C}$.
This is the live narrowing of the W5 brainstorm list (PTO / structured noise / Tijana remain deferred candidates).
Wenbin (H2 / W5): ``no recalibration, strategy: data-randomize to achieve validity guarantee, this answers (iii).''
Repo location of companion notes (not live math): \texttt{Formulation/research}.}
...
\annote{Decision: cite the \texttt{Work/research-results} operator library as an existence / supporting prior only (usefulness~A) for data-randomize / selection-under-perturbation --- acknowledge \texttt{LEAN\_FULL} certificates, do not dump proofs, and do not import those operators as live Wenbin-UQ equations.
Purpose: show that selection-under-perturbation is a real, already-certified object class that can motivate the~(iii) randomization path; not a second problem definition.}
