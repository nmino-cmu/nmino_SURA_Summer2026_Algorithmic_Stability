...
cd "$ROOT"
OID="$1"; LEAN="$2"; DISPLAY="$3"; KIND="$4"
PKG="${OID//-/_}"
case "$KIND" in
  clamp) THM="${OID}-clamp-stability" ;;
  feasible) THM="${OID}-feasible-ball-identity" ;;
  conjunction) THM="${OID}-conjunction-preservation" ;;
  disjunction) THM="${OID}-disjunction-preservation" ;;
  *) echo "bad kind $KIND"; exit 2 ;;
esac
export OID THM
...
python3 scripts/clone_projection_op.py --id "$OID" --lean "$LEAN" --display "$DISPLAY" --kind "$KIND"
...
python3 scripts/write_projection_package.py "$OID" "$LEAN" "$DISPLAY" "$KIND" "$THM"
...
python3 scripts/sync_projection_metadata.py "$OID" "$THM"
...
git add -A
git commit -m "feat(operator): add ${OID} via Projection ${KIND} core (LEAN_FULL)"
git push
