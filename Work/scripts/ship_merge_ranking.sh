...
python3 scripts/clone_ranking_op.py --id "$OID" --lean "$LEAN" --display "$DISPLAY"
...
python3 scripts/write_ranking_package.py "$OID" "$LEAN" "$DISPLAY"
...
git add -A
git commit -m "feat(operator): add ${OID} via OrderStat ranking-margin (LEAN_FULL)"
git push
