#!/usr/bin/env python3
"""Clone top-k ranking-margin package into a new ranking-alias operator and wire it."""
...
def main() -> int:
    ...
    stmt = (
        ...
        f"indices i,j: s_i < s_j iff (s+δ)_i < (s+δ)_j. ({display} uses this ranking core.)"
    )
    sharp_stmt = (
    ...
    (dst / "math.py").write_text(
        textwrap.dedent(
            f'''
            """{display} — ranking-margin core (Mathlib R)."""
            from __future__ import annotations
    ...
    (dst / "verify.py").write_text(
        textwrap.dedent(
            ...
            def verify_{pkg}_margin(claim):
                if not claim_is_{pkg}_margin(claim): return VerifyResult(False,"claim_mismatch")
    ...
    (dst / "discovery.py").write_text(
        textwrap.dedent(
            ...
            def discover_{pkg}(ir: DiscoveryIR) -> dict[str, Any]:
                op = a_engines.run_operator_analyzer(ir, {{"name": OPERATOR, "form": "{display} via ranking margin", "reduction": "order_stat_ranking"}})
                ...
                assum = a_engines.run_assumptions(ir, "Finite n≥2; pairwise gaps; ℓ∞ perturbations.")
                claim = {{"statement": THEOREM_STATEMENT, "chain_segment": "inference", "operator": OPERATOR, "theorem_id": THEOREM_ID, "evaluation": EVALUATION_METHOD, "sharpness_statement": SHARPNESS_STATEMENT, "formal": {formal!r}}}
                ...
                cex = a_engines.run_conjecture(ir, "Ranking margin sharp via collision adversary.")
                soft = a_engines.run_soft_attack(ir, tip)
                port = a_engines.run_pareto_portfolio(ir, [tip])
                return {{"operator": op.version_ids[0], "instability": instab.version_ids[0], "quantity": qty.version_ids[0], "mechanism": mech.version_ids[0], "psi": psi.version_ids[0], "assumptions": assum.version_ids[0], "theorem": tip, "proof_sketch": sketch.version_ids[0], "bridge": bridge.version_ids[0], "utility": util.version_ids[0], "open_questions": open_q.version_ids[0], "sharpness_conjecture": cex.version_ids[0], "soft_attack": soft.version_ids, "portfolio": port.version_ids[0], "claim_payload": claim}}
            '''
        ).lstrip(),
        encoding="utf-8",
    )
    (dst / "lean_profile.py").write_text(
        textwrap.dedent(
            ...
            TARGETS = [
              {{"target_id":"preservation","prop_fully_qualified":"Research.Operators.{lean}.Preservation.{prop}","theorem_name":"{thm}","lemma_deps":[]}},
              {{"target_id":"sharpness","prop_fully_qualified":"Research.Operators.{lean}.Preservation.{sharp}","theorem_name":"{thm_s}","lemma_deps":[]}},
            ]
            ...
            PROP_DEPS = (Path("Research/Operators/{lean}/Preservation.lean"), Path("Research/Operators/OrderStat/Ranking.lean"), Path("Research/Operators/OrderStat/KthMargin.lean"), Path("Research/Operators/OrderStat/Basic.lean"), Path("Research/Operators/Argmax/Basic.lean"))
    ...
    (dst / "__init__.py").write_text(
        textwrap.dedent(
            ...
            __all__ = ["EVALUATION_METHOD","OPERATOR","THEOREM_ID","THEOREM_STATEMENT","claim_is_{pkg}_margin","verify_{pkg}_margin","run_{pkg}_margin_workflow"]
            '''
        ).lstrip(),
        encoding="utf-8",
    )

    lean_path = ROOT / f"lean/Research/Operators/{lean}/Preservation.lean"
    ...
    (ROOT / f"implementation/tests/test_{pkg}_operator.py").write_text(
        textwrap.dedent(
            ...
            def test_verifier():
                ...
                vr=verify_{pkg}_margin(claim); assert vr.ok,(vr.detail,vr.counterexamples)
            '''
        ).lstrip(),
        encoding="utf-8",
    )
    ...
    (ROOT / f"implementation/docs/operators/{oid}.md").write_text(
        f"# {display}\n\nRanking-margin core. Lean: `Research.Operators.{lean}.Preservation`.\n",
        encoding="utf-8",
    )
    ...
    # profiles: append import + tuple entry
...
'''
    ...
    if f"{pkg}_profile" not in rt:
        rt = rt.replace(
            "from operators.top_k import lean_profile as top_k_profile  # noqa: E402\n",
            "from operators.top_k import lean_profile as top_k_profile  # noqa: E402\n"
            f"from operators.{pkg} import lean_profile as {pkg}_profile  # noqa: E402\n",
        )
        rt = rt.replace(
            '    ("top-k", "top-k-margin", top_k_profile),\n)',
            f'    ("top-k", "top-k-margin", top_k_profile),\n    ("{oid}", "{thm_id}", {pkg}_profile),\n)',
        )
