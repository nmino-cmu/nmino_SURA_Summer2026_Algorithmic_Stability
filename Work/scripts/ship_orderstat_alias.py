#!/usr/bin/env python3
"""Ship an OrderStat-KthMargin alias operator end-to-end (files → e2e → PDF → registry).
...
def main() -> int:
    ...
    stmt = (
        f"Let n≥2, s∈ℤ^n, k∈ℕ with k<n, ε≥0, and i a unique strict k-th smallest index "
        f"(exactly k scores strictly below s_i and no other index shares s_i). "
        f"If every pairwise gap exceeds 2ε and ‖δ‖_∞≤ε, then i remains the unique strict "
        f"k-th smallest of s+δ. ({args.display} selection uses this k-th order statistic core.)"
    )
    sharp_stmt = (
        "If some rival j≠i has |s_i−s_j|≤2ε, there exists ‖δ‖_∞≤ε destroying unique "
        "strict k-th-smallest status of i."
    )
    formal = {
        "selection": "unique_strict_kth_smallest",
    ...
    write(
        ...
        /- STATEMENT_BEGIN -/
        def {prop} : Prop := KthMarginInvarianceProp
        def {sharp} : Prop := KthMarginSharpnessProp
        /- STATEMENT_END -/

        theorem {thm} : {prop} := kth_margin_invariance
        theorem {thm_s} : {sharp} := kth_margin_sharpness
...
'''
    ...
    write(
        ...
        def discover_{pkg}(ir: DiscoveryIR) -> dict[str, Any]:
            op = a_engines.run_operator_analyzer(ir, {{"name": OPERATOR, "form": "{args.display} as strict kth", "reduction": "order_stat_kth_margin"}})
