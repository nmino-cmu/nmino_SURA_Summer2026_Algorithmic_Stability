...
{\normalsize\texttt{operator/coordinate-clipping}}}
\date{2026-07-29}
...
\begin{abstract}
\leanpath{coordinate-clipping} uses interval clamp, which is $1$-Lipschitz; hence $|x'-x|\le\varepsilon$ implies the clamped values move by at most $\varepsilon$. Lean \texttt{LEAN\_FULL}.
\end{abstract}
...
\section{Problem}
{\raggedright\sloppy
\leanpath{coordinate-clipping} projects coordinates onto intervals via $\mathrm{clamp}(x;\mathrm{lo},\mathrm{hi})=\max(\mathrm{lo},\min(x,\mathrm{hi}))$.
\par}
...
\item[Lean module] \leanpath{Research.Operators.CoordinateClipping.Preservation}
\item[Source file] \leanpath{lean/Research/Operators/CoordinateClipping/Preservation.lean}
\item[Theorems]\leavevmode\par\vspace{0.15em}\begin{itemize}[leftmargin=1.2em]
  \item \leanpath{coordinate\_clipping\_clamp\_stability}
  \item \leanpath{coordinate\_clipping\_clamp\_sharpness}
\end{itemize}
\item[Note] Uses Projection.Clamp.
\item[Certificate] \leanpath{lean/certificates/coordinate-clipping/coordinate-clipping-clamp-stability}
