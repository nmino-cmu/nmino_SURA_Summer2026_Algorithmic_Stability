...
\title{Phase B selection-stability packaging for argmax under ell-infinity score ball}
\author{Research Architecture Runtime\\[0.25em]
{\normalsize\texttt{operator/argmax}}}
\date{2026-07-29}
...
\begin{abstract}
\texttt{argmax} selects a unique maximizer. Margin $\gamma(s)>2\varepsilon$ is necessary and sufficient for unique-max invariance under $\|\delta\|_\infty\le\varepsilon$. Lean \texttt{LEAN\_FULL} (reduction to \leanpath{Argmax.Margin}).
\end{abstract}
...
\item[Lean module] \leanpath{Research.Operators.Argmax.Margin}
\item[Source file] \leanpath{lean/Research/Operators/Argmax/Margin.lean}
\item[Theorems]\leavevmode\par\vspace{0.15em}\begin{itemize}[leftmargin=1.2em]
  \item \leanpath{margin\_invariance}
  \item \leanpath{margin\_sharpness}
\end{itemize}
\item[Note] Definitional reduction of Argmax.Margin.
\item[Math authority] \leanpath{research-results/argmax/bounded-perturbation-margin}
\item[Certificate] \leanpath{lean/certificates/argmax/bounded-perturbation-margin}
...
{\raggedright\sloppy
\begin{itemize}
\item Authority: \texttt{argmax/bounded-perturbation-margin}.
\item \leanpath{Argmax.Margin}.
\end{itemize}
\par}
