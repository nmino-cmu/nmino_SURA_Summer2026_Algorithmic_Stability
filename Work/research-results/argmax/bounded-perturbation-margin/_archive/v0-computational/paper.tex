...
\author{Research Architecture Runtime \\ \texttt{feature/argmax}}
\date{2026-07-25}
...
Verification in the repository is \textbf{computational}
(\texttt{ARGMAX\_MARGIN\_COMPUTATIONAL\_V1}), not a Lean/ITP formalization.
\end{abstract}
...
\section{Limitations}
\begin{itemize}
\item Verification in-repo is computational certification, not Lean/ITP.
\item Finite score vectors; abstract \(s\), not \(s_i(D)\).
\item \(\ell_\infty\) only; other norms change constants.
\item Floating-point evaluation can blur exact equality \(\gamma=2\varepsilon\).
\end{itemize}
...
Artifacts are minted as typed Discovery IR, packaged as a sealed CRP, and
discharged by System~B via \texttt{operators.argmax.verify} with evaluation
method \texttt{ARGMAX\_MARGIN\_COMPUTATIONAL\_V1}.

\section{Verifier summary}
End-to-end workflow reports audit verdict PASS with no unresolved obligations
when the sealed claim statement exactly matches the canonical theorem text and
formal fields. Feedback limitations include
\texttt{COMPUTATIONAL\_VERIFICATION\_NOT\_LEAN}.
