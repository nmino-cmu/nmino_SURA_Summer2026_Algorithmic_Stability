...
\title{Hierarchical maximum Margin Preservation under Bounded Score Perturbations}
\author{Research Architecture Runtime\\[0.25em]
{\normalsize\texttt{operator/hierarchical-maximum}}}
\date{2026-07-29}
...
\begin{abstract}
\leanpath{hierarchical-maximum} selects a unique maximizer. Margin $\gamma(s)>2\varepsilon$ is necessary and sufficient for unique-max invariance under $\|\delta\|_\infty\le\varepsilon$. Lean \texttt{LEAN\_FULL} (reduction to \texttt{Argmax.Margin}).
\end{abstract}
...
\section{Problem}
{\raggedright\sloppy
The \leanpath{hierarchical-maximum} operator returns the unique maximizer index $i^\star$ of scores $s\in\mathbb{R}^m$ ($m\ge 2$), up to the operator's definitional presentation (heap top, tournament winner, masked max, etc.).
\par}
...
\item[Lean module] \leanpath{Research.Operators.HierarchicalMaximum.Margin}
\item[Source file] \leanpath{lean/Research/Operators/HierarchicalMaximum/Margin.lean}
\item[Theorems]\leavevmode\par\vspace{0.15em}\begin{itemize}[leftmargin=1.2em]
  \item \leanpath{hierarchical\_maximum\_margin\_invariance}
  \item \leanpath{hierarchical\_maximum\_margin\_sharpness}
\end{itemize}
\item[Note] Definitional reduction of Argmax.Margin.
\item[Certificate] \leanpath{lean/certificates/hierarchical-maximum/hierarchical-maximum-margin}
...
{\raggedright\sloppy
Any unique-max presentation of \leanpath{hierarchical-maximum} inherits Argmax margin stability. Related: \leanpath{argmax}, \leanpath{tournament-winner}, \leanpath{greedy-maximum-selection}.
\par}
