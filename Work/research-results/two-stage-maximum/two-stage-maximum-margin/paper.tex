...
\title{Two-stage maximum Margin Preservation under Bounded Score Perturbations}
\author{Research Architecture Runtime\\[0.25em]
{\normalsize\texttt{operator/two-stage-maximum}}}
\date{2026-07-29}
...
\begin{abstract}
\leanpath{two-stage-maximum} selects a unique maximizer. Margin $\gamma(s)>2\varepsilon$ is necessary and sufficient for unique-max invariance under $\|\delta\|_\infty\le\varepsilon$. Lean \texttt{LEAN\_FULL} (reduction to \texttt{Argmax.Margin}).
\end{abstract}
...
\section{Problem}
{\raggedright\sloppy
The \leanpath{two-stage-maximum} operator returns the unique maximizer index $i^\star$ of scores $s\in\mathbb{R}^m$ ($m\ge 2$), up to the operator's definitional presentation (heap top, tournament winner, masked max, etc.).
\par}
...
\item[Lean module] \leanpath{Research.Operators.TwoStageMaximum.Margin}
\item[Source file] \leanpath{lean/Research/Operators/TwoStageMaximum/Margin.lean}
\item[Theorems]\leavevmode\par\vspace{0.15em}\begin{itemize}[leftmargin=1.2em]
  \item \leanpath{two\_stage\_maximum\_margin\_invariance}
  \item \leanpath{two\_stage\_maximum\_margin\_sharpness}
\end{itemize}
\item[Note] Definitional reduction of Argmax.Margin.
\item[Certificate] \leanpath{lean/certificates/two-stage-maximum/two-stage-maximum-margin}
...
{\raggedright\sloppy
Any unique-max presentation of \leanpath{two-stage-maximum} inherits Argmax margin stability. Related: \leanpath{argmax}, \leanpath{tournament-winner}, \leanpath{greedy-maximum-selection}.
\par}
