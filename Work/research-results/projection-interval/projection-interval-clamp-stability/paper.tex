...
{\normalsize\texttt{operator/projection-interval}}}
\date{2026-07-29}
...
\begin{abstract}
\leanpath{projection-interval} uses interval clamp, which is $1$-Lipschitz; hence $|x'-x|\le\varepsilon$ implies the clamped values move by at most $\varepsilon$. Lean \texttt{LEAN\_FULL}.
\end{abstract}
...
\section{Problem}
{\raggedright\sloppy
\leanpath{projection-interval} projects coordinates onto intervals via $\mathrm{clamp}(x;\mathrm{lo},\mathrm{hi})=\max(\mathrm{lo},\min(x,\mathrm{hi}))$.
\par}
...
\item[Lean module] \leanpath{Research.Operators.ProjectionInterval.Preservation}
\item[Source file] \leanpath{lean/Research/Operators/ProjectionInterval/Preservation.lean}
\item[Theorems]\leavevmode\par\vspace{0.15em}\begin{itemize}[leftmargin=1.2em]
  \item \leanpath{projection\_interval\_clamp\_stability}
  \item \leanpath{projection\_interval\_clamp\_sharpness}
\end{itemize}
\item[Note] Uses Projection.Clamp.
\item[Certificate] \leanpath{lean/certificates/projection-interval/projection-interval-clamp-stability}
