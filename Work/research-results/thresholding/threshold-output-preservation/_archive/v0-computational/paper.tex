...
\author{Research Architecture Runtime \\ \texttt{feature/thresholding}}
\date{2026-07-25}
...
counterexamples.
Repository verification is computational
(\texttt{THRESHOLD\_PRESERVATION\_COMPUTATIONAL\_V1}), not Lean/ITP.
\end{abstract}
...
\section{Limitations}
\begin{itemize}
\item Computational certification, not Lean/ITP.
...
Discovery IR \(\to\) sealed CRP \(\to\) System~B discharger
\texttt{THRESHOLD\_PRESERVATION\_COMPUTATIONAL\_V1}.

\section{Verifier summary}
End-to-end workflow reports audit verdict PASS with no unresolved obligations
when the sealed claim matches the canonical statement and formal fields.
Limitations include \texttt{COMPUTATIONAL\_VERIFICATION\_NOT\_LEAN}.
