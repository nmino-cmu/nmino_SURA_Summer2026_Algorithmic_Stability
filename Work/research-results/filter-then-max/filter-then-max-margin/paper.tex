...
\title{Filter-then-max Margin Preservation under Bounded Score Perturbations}
\author{Research Architecture Runtime\\[0.25em]
{\normalsize\texttt{operator/filter-then-max}}}
\date{2026-07-29}
...
\begin{abstract}
\leanpath{filter-then-max} selects a unique maximizer. Margin $\gamma(s)>2\varepsilon$ is necessary and sufficient for unique-max invariance under $\|\delta\|_\infty\le\varepsilon$. Lean \texttt{LEAN\_FULL} (reduction to \texttt{Argmax.Margin}).
\end{abstract}
...
\section{Problem}
{\raggedright\sloppy
The \leanpath{filter-then-max} operator returns the unique maximizer index $i^\star$ of scores $s\in\mathbb{R}^m$ ($m\ge 2$), up to the operator's definitional presentation (heap top, tournament winner, masked max, etc.).
\par}
...
\item[Lean module] \leanpath{Research.Operators.FilterThenMax.Margin}
\item[Source file] \leanpath{lean/Research/Operators/FilterThenMax/Margin.lean}
\item[Theorems]\leavevmode\par\vspace{0.15em}\begin{itemize}[leftmargin=1.2em]
  \item \leanpath{filter\_then\_max\_margin\_invariance}
  \item \leanpath{filter\_then\_max\_margin\_sharpness}
\end{itemize}
\item[Note] Definitional reduction of Argmax.Margin.
\item[Certificate] \leanpath{lean/certificates/filter-then-max/filter-then-max-margin}
...
{\raggedright\sloppy
Any unique-max presentation of \leanpath{filter-then-max} inherits Argmax margin stability. Related: \leanpath{argmax}, \leanpath{tournament-winner}, \leanpath{greedy-maximum-selection}.
\par}
