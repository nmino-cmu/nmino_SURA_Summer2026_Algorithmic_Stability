...
from operators.best_first_node_selection.discovery import discover_best_first_node_selection
...
def run_best_first_node_selection_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_best_first_node_selection(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("best-first-node-selection", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("best-first-node-selection", "PHASE_A_CHARACTERIZATION", "best-first-node-selection-margin")
