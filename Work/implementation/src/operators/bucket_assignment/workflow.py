...
from operators.bucket_assignment.discovery import discover_bucket_assignment
...
def run_bucket_assignment_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_bucket_assignment(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("bucket-assignment", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("bucket-assignment", "PHASE_A_CHARACTERIZATION", "bucket-assignment-margin")
