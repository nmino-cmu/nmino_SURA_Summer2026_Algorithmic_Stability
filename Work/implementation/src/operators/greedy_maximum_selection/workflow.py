...
from operators.greedy_maximum_selection.discovery import discover_greedy_maximum_selection
...
def run_greedy_maximum_selection_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_greedy_maximum_selection(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("greedy-maximum-selection", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("greedy-maximum-selection", "PHASE_A_CHARACTERIZATION", "greedy-maximum-selection-margin")
