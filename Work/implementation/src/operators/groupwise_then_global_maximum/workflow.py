...
from operators.groupwise_then_global_maximum.discovery import discover_groupwise_then_global_maximum
...
def run_groupwise_then_global_maximum_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_groupwise_then_global_maximum(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("groupwise-then-global-maximum", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("groupwise-then-global-maximum", "PHASE_A_CHARACTERIZATION", "groupwise-then-global-maximum-margin")
