...
def discover_groupwise_then_global_maximum(ir: DiscoveryIR) -> dict[str, Any]:
    op = a_engines.run_operator_analyzer(
        ir,
        {"name": OPERATOR, "form": "Groupwise-then-global maximum via Argmax margin", "reduction": "argmax_margin"},
    )
