...
from operators.masked_top_k.discovery import discover_masked_top_k
...
def run_masked_top_k_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_masked_top_k(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("masked-top-k", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("masked-top-k", "PHASE_A_CHARACTERIZATION", "masked-top-k-margin")
