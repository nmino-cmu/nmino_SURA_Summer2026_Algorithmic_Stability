...
from operators.multi_criteria_lexicographic.discovery import discover_multi_criteria_lexicographic
...
def run_multi_criteria_lexicographic_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_multi_criteria_lexicographic(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("multi-criteria-lexicographic", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("multi-criteria-lexicographic", "PHASE_A_CHARACTERIZATION", "multi-criteria-lexicographic-margin")
