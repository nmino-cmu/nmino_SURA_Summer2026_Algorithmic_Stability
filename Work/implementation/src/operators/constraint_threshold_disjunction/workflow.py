...
from operators.constraint_threshold_disjunction.discovery import discover_constraint_threshold_disjunction
...
def run_constraint_threshold_disjunction_preservation_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_constraint_threshold_disjunction(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("constraint-threshold-disjunction", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("constraint-threshold-disjunction", "PHASE_A_CHARACTERIZATION", "constraint-threshold-disjunction-disjunction-preservation")
