...
from operators.priority_queue_maximum.discovery import discover_priority_queue_maximum
...
def run_priority_queue_maximum_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_priority_queue_maximum(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("priority-queue-maximum", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("priority-queue-maximum", "PHASE_A_CHARACTERIZATION", "priority-queue-maximum-margin")
