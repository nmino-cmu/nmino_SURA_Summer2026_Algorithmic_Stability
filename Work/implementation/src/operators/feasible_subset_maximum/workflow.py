...
from operators.feasible_subset_maximum.discovery import discover_feasible_subset_maximum
...
def run_feasible_subset_maximum_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_feasible_subset_maximum(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("feasible-subset-maximum", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("feasible-subset-maximum", "PHASE_A_CHARACTERIZATION", "feasible-subset-maximum-margin")
