...
from operators.top_k_then_threshold.math import (
...
def claim_is_top_k_then_threshold_margin(claim):
    return claim.get("operator")==OPERATOR and claim.get("theorem_id")==THEOREM_ID and claim.get("evaluation")==EVALUATION_METHOD

def verify_top_k_then_threshold_margin(claim):
    if not claim_is_top_k_then_threshold_margin(claim): return VerifyResult(False,"claim_mismatch")
