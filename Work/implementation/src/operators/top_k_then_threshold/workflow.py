...
from operators.top_k_then_threshold.discovery import discover_top_k_then_threshold
...
def run_top_k_then_threshold_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_top_k_then_threshold(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("top-k-then-threshold", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("top-k-then-threshold", "PHASE_A_CHARACTERIZATION", "top-k-then-threshold-margin")
