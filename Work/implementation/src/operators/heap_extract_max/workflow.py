...
from operators.heap_extract_max.discovery import discover_heap_extract_max
...
def run_heap_extract_max_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_heap_extract_max(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("heap-extract-max", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("heap-extract-max", "PHASE_A_CHARACTERIZATION", "heap-extract-max-margin")
