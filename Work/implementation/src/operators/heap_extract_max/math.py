"""Heap extract-max - unique-max Argmax-margin reduction (Real)."""
...
EVALUATION_METHOD = "HEAP_EXTRACT_MAX_MARGIN_COMPUTATIONAL_V1"
THEOREM_STATEMENT = ('Let m>=2 and let scores be constructed by (heap key scores with unique maximizer). If i* is the unique maximizer with margin gamma>2*epsilon and ||delta||_inf<=epsilon, then i* remains the unique maximizer after perturbation (Heap extract-max reduces to Argmax margin).')
