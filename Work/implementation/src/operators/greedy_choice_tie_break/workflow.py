...
from operators.greedy_choice_tie_break.discovery import discover_greedy_choice_tie_break
...
def run_greedy_choice_tie_break_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_greedy_choice_tie_break(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("greedy-choice-tie-break", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("greedy-choice-tie-break", "PHASE_A_CHARACTERIZATION", "greedy-choice-tie-break-margin")
