...
from operators.weighted_tournament_winner.discovery import discover_weighted_tournament_winner
...
def run_weighted_tournament_winner_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_weighted_tournament_winner(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("weighted-tournament-winner", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("weighted-tournament-winner", "PHASE_A_CHARACTERIZATION", "weighted-tournament-winner-margin")
