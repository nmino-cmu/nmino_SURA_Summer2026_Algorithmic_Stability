"""Weighted tournament winner - unique-max Argmax-margin reduction (Real)."""
...
EVALUATION_METHOD = "WEIGHTED_TOURNAMENT_WINNER_MARGIN_COMPUTATIONAL_V1"
THEOREM_STATEMENT = ('Let m>=2 and let scores be constructed by (weighted tournament scores with unique maximizer). If i* is the unique maximizer with margin gamma>2*epsilon and ||delta||_inf<=epsilon, then i* remains the unique maximizer after perturbation (Weighted tournament winner reduces to Argmax margin).')
