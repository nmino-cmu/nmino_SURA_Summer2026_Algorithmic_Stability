...
from operators.filter_then_max.discovery import discover_filter_then_max
...
def run_filter_then_max_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_filter_then_max(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("filter-then-max", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("filter-then-max", "PHASE_A_CHARACTERIZATION", "filter-then-max-margin")
