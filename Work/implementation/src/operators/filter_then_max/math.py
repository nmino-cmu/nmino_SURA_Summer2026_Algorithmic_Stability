"""Filter-then-max - unique-max Argmax-margin reduction (Real)."""
...
EVALUATION_METHOD = "FILTER_THEN_MAX_MARGIN_COMPUTATIONAL_V1"
THEOREM_STATEMENT = ('Let m>=2 and let scores be constructed by (filtered candidate scores with unique maximizer). If i* is the unique maximizer with margin gamma>2*epsilon and ||delta||_inf<=epsilon, then i* remains the unique maximizer after perturbation (Filter-then-max reduces to Argmax margin).')
