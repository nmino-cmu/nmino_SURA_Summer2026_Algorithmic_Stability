...
from operators.percentile.discovery import discover_percentile
...
def run_percentile_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    orch = DiscoveryOrchestrator.create()
    orch.scope_pin = SCOPE
    orch.principal = PRINCIPAL
    for s in (State.DS01, State.DS02, State.DS03):
        orch.advance(s)
    tips = discover_percentile(orch.ir)
    th = tips["theorem"]
    orch.advance(State.DS05)
    orch.ir.upsert_branch("percentile", [th])
    ...
    vid = orch.compile_portfolio_member("percentile", "PHASE_A_CHARACTERIZATION", "percentile-margin")
