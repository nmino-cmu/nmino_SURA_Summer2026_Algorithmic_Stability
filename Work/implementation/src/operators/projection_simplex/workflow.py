...
from operators.projection_simplex.discovery import discover_projection_simplex
...
def run_projection_simplex_preservation_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_projection_simplex(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("projection-simplex", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("projection-simplex", "PHASE_A_CHARACTERIZATION", "projection-simplex-feasible-ball-identity")
