...
def claim_is_quantile_margin(claim: dict[str, Any]) -> bool:
    return (
...
def verify_quantile_margin(claim: dict[str, Any]) -> VerifyResult:
    if not claim_is_quantile_margin(claim):
    ...
    for _ in range(300):
        n = rng.randint(2, 7)
        # distinct scores with controlled gaps
        ...
        # sharpness: pick rival with gap <= 2eps
        ...
        if is_strict_kth(news, k, i):
            failures.append({"kind": "break_failed", "scores": scores, "eps": eps})
    if failures:
