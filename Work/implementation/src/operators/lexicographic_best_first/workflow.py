...
from operators.lexicographic_best_first.discovery import discover_lexicographic_best_first
...
def run_lexicographic_best_first_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_lexicographic_best_first(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("lexicographic-best-first", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("lexicographic-best-first", "PHASE_A_CHARACTERIZATION", "lexicographic-best-first-margin")
