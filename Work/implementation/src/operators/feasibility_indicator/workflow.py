...
from operators.feasibility_indicator.discovery import discover_feasibility_indicator
...
def run_feasibility_indicator_preservation_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_feasibility_indicator(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("feasibility-indicator", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("feasibility-indicator", "PHASE_A_CHARACTERIZATION", "feasibility-indicator-feasible-ball-identity")
