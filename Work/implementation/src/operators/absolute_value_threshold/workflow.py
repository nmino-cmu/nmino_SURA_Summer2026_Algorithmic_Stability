...
from operators.absolute_value_threshold.discovery import discover_abs_threshold
...
@dataclass
class AbsThresholdResearchResult:
    session_id: str
...
def run_abs_threshold_preservation_workflow(*, export_lean_bundle: bool = False) -> AbsThresholdResearchResult:
    orch = DiscoveryOrchestrator.create()
    orch.scope_pin = SCOPE
    orch.principal = PRINCIPAL
    for s in (State.DS01, State.DS02, State.DS03):
        orch.advance(s)
    tips = discover_abs_threshold(orch.ir)
    th = tips["theorem"]
    orch.advance(State.DS05)
    orch.ir.upsert_branch("absolute-value-threshold", [th])
    ...
    vid = orch.compile_portfolio_member(
        "absolute-value-threshold", "PHASE_A_CHARACTERIZATION", "abs-threshold-preservation"
    )
    ...
    return AbsThresholdResearchResult(
