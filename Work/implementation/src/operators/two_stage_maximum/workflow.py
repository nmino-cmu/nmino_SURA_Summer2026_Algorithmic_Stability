...
from operators.two_stage_maximum.discovery import discover_two_stage_maximum
...
def run_two_stage_maximum_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_two_stage_maximum(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("two-stage-maximum", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("two-stage-maximum", "PHASE_A_CHARACTERIZATION", "two-stage-maximum-margin")
