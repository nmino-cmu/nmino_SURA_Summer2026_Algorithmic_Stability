"""Two-stage maximum - unique-max Argmax-margin reduction (Real)."""
...
EVALUATION_METHOD = "TWO_STAGE_MAXIMUM_MARGIN_COMPUTATIONAL_V1"
THEOREM_STATEMENT = ('Let m>=2 and let scores be constructed by (two-stage scores with unique maximizer). If i* is the unique maximizer with margin gamma>2*epsilon and ||delta||_inf<=epsilon, then i* remains the unique maximizer after perturbation (Two-stage maximum reduces to Argmax margin).')
