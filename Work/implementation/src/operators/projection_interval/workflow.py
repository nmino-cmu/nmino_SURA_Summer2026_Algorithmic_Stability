...
from operators.projection_interval.discovery import discover_projection_interval
...
def run_projection_interval_preservation_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_projection_interval(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("projection-interval", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("projection-interval", "PHASE_A_CHARACTERIZATION", "projection-interval-clamp-stability")
