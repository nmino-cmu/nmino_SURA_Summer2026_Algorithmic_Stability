...
from operators.constraint_threshold_conjunction.discovery import discover_constraint_threshold_conjunction
...
def run_constraint_threshold_conjunction_preservation_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_constraint_threshold_conjunction(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("constraint-threshold-conjunction", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("constraint-threshold-conjunction", "PHASE_A_CHARACTERIZATION", "constraint-threshold-conjunction-conjunction-preservation")
