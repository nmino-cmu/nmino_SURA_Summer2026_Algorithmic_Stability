...
from operators.projection_l2_ball.math import (
...
def discover_projection_l2_ball(ir: DiscoveryIR) -> dict[str, Any]:
    op = a_engines.run_operator_analyzer(
        ir, {"name": OPERATOR, "form": "Projection onto the l2 ball", "kind": "feasible"}
    )
    ...
    assum = a_engines.run_assumptions(ir, "Mathlib ℝ core; abs-ball perturbations.")
    claim = {
        ...
        "formal": {'operator_kind': 'feasible_ball_identity', 'perturbation': 'abs_ball', 'property': 'identity_on_epsilon_interior'},
    }
    tip = ir.mint(
