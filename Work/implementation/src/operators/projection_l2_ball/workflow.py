...
from operators.projection_l2_ball.discovery import discover_projection_l2_ball
...
def run_projection_l2_ball_preservation_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_projection_l2_ball(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("projection-l2-ball", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("projection-l2-ball", "PHASE_A_CHARACTERIZATION", "projection-l2-ball-feasible-ball-identity")
