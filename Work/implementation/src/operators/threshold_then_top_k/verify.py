...
from operators.threshold_then_top_k.math import (
...
def claim_is_threshold_then_top_k_margin(claim):
    return claim.get("operator")==OPERATOR and claim.get("theorem_id")==THEOREM_ID and claim.get("evaluation")==EVALUATION_METHOD

def verify_threshold_then_top_k_margin(claim):
    if not claim_is_threshold_then_top_k_margin(claim): return VerifyResult(False,"claim_mismatch")
