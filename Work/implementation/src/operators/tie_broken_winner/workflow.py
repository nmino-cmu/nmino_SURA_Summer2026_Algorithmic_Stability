...
from operators.tie_broken_winner.discovery import discover_tie_broken_winner
...
def run_tie_broken_winner_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_tie_broken_winner(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("tie-broken-winner", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("tie-broken-winner", "PHASE_A_CHARACTERIZATION", "tie-broken-winner-margin")
