...
from operators.hierarchical_maximum.discovery import discover_hierarchical_maximum
...
def run_hierarchical_maximum_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_hierarchical_maximum(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("hierarchical-maximum", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("hierarchical-maximum", "PHASE_A_CHARACTERIZATION", "hierarchical-maximum-margin")
