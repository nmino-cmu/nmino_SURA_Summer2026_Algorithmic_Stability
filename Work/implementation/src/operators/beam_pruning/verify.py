...
from operators.beam_pruning.math import (
...
def claim_is_beam_pruning_margin(claim):
    return claim.get("operator")==OPERATOR and claim.get("theorem_id")==THEOREM_ID and claim.get("evaluation")==EVALUATION_METHOD

def verify_beam_pruning_margin(claim):
    if not claim_is_beam_pruning_margin(claim): return VerifyResult(False,"claim_mismatch")
