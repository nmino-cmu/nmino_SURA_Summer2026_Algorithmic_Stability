...
from operators.beam_pruning.discovery import discover_beam_pruning
...
def run_beam_pruning_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_beam_pruning(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("beam-pruning", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("beam-pruning", "PHASE_A_CHARACTERIZATION", "beam-pruning-margin")
