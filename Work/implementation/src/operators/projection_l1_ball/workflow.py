...
from operators.projection_l1_ball.discovery import discover_projection_l1_ball
...
def run_projection_l1_ball_preservation_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_projection_l1_ball(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("projection-l1-ball", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("projection-l1-ball", "PHASE_A_CHARACTERIZATION", "projection-l1-ball-feasible-ball-identity")
