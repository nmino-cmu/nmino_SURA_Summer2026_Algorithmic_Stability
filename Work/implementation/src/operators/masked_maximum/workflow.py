...
from operators.masked_maximum.discovery import discover_masked_maximum
...
def run_masked_maximum_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_masked_maximum(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("masked-maximum", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("masked-maximum", "PHASE_A_CHARACTERIZATION", "masked-maximum-margin")
