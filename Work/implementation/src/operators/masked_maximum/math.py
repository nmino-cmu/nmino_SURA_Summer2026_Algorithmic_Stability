"""Masked maximum - unique-max Argmax-margin reduction (Real)."""
...
EVALUATION_METHOD = "MASKED_MAXIMUM_MARGIN_COMPUTATIONAL_V1"
THEOREM_STATEMENT = ('Let m>=2 and let scores be constructed by (masked candidate scores with unique maximizer). If i* is the unique maximizer with margin gamma>2*epsilon and ||delta||_inf<=epsilon, then i* remains the unique maximizer after perturbation (Masked maximum reduces to Argmax margin).')
