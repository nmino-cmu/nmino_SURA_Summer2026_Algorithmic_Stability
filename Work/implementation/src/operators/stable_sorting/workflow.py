...
from operators.stable_sorting.discovery import discover_stable_sorting
...
def run_stable_sorting_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_stable_sorting(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("stable-sorting", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("stable-sorting", "PHASE_A_CHARACTERIZATION", "stable-sorting-margin")
