...
from operators.stable_sorting.math import (
...
def claim_is_stable_sorting_margin(claim):
    return claim.get("operator")==OPERATOR and claim.get("theorem_id")==THEOREM_ID and claim.get("evaluation")==EVALUATION_METHOD

def verify_stable_sorting_margin(claim):
    if not claim_is_stable_sorting_margin(claim): return VerifyResult(False,"claim_mismatch")
