...
from operators.lexicographic_ordering.discovery import discover_lexicographic_ordering
...
def run_lexicographic_ordering_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_lexicographic_ordering(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("lexicographic-ordering", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("lexicographic-ordering", "PHASE_A_CHARACTERIZATION", "lexicographic-ordering-margin")
