...
from operators.kth_order_statistic.math import EVALUATION_METHOD, OPERATOR, SHARPNESS_STATEMENT, THEOREM_ID, THEOREM_STATEMENT
from system_a import engines as a_engines
from system_a.ir import DiscoveryIR
from system_a.ownership import CLASS_OWNER

def discover_kth_order_statistic(ir: DiscoveryIR) -> dict[str, Any]:
    op = a_engines.run_operator_analyzer(ir, {"name": OPERATOR, "form": "k-th order statistic as strict kth", "reduction": "order_stat_kth_margin"})
    ...
    claim = {
        ...
        "formal": {'selection': 'unique_strict_kth_smallest', 'perturbation': 'linf_ball', 'gap_condition': 'all_pairwise_gaps > 2*epsilon'},
    }
