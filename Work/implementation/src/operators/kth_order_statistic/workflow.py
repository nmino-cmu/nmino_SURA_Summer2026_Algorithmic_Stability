...
from operators.kth_order_statistic.discovery import discover_kth_order_statistic
...
def run_kth_order_statistic_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    orch = DiscoveryOrchestrator.create()
    orch.scope_pin = SCOPE
    orch.principal = PRINCIPAL
    for s in (State.DS01, State.DS02, State.DS03):
        orch.advance(s)
    tips = discover_kth_order_statistic(orch.ir)
    th = tips["theorem"]
    orch.advance(State.DS05)
    orch.ir.upsert_branch("kth-order-statistic", [th])
    ...
    vid = orch.compile_portfolio_member("kth-order-statistic", "PHASE_A_CHARACTERIZATION", "kth-order-statistic-margin")
