...
from operators.stable_partition_threshold.discovery import discover_stable_partition_threshold
...
def run_stable_partition_threshold_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_stable_partition_threshold(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("stable-partition-threshold", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("stable-partition-threshold", "PHASE_A_CHARACTERIZATION", "stable-partition-threshold-margin")
