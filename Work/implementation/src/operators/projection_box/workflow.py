...
from operators.projection_box.discovery import discover_projection_box
...
def run_projection_box_preservation_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_projection_box(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("projection-box", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("projection-box", "PHASE_A_CHARACTERIZATION", "projection-box-clamp-stability")
