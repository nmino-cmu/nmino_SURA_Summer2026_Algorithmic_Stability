...
from operators.weighted_score_selection.discovery import discover_weighted_score_selection
...
def run_weighted_score_selection_margin_workflow(*, export_lean_bundle: bool = False) -> ResearchResult:
    ...
    tips = discover_weighted_score_selection(orch.ir); th = tips["theorem"]; orch.advance(State.DS05)
    orch.ir.upsert_branch("weighted-score-selection", [th]); orch.ir.add_dep(th, tips["assumptions"], "depends"); orch.ir.add_dep(th, tips["proof_sketch"], "depends")
    orch.advance(State.DS07); orch.advance(State.DS08)
    vid = orch.compile_portfolio_member("weighted-score-selection", "PHASE_A_CHARACTERIZATION", "weighted-score-selection-margin")
